...
import { Trash, ArrowLeft } from '@tamagui/lucide-icons'
...
export default function DayExpensesScreen() {
  ...
  const formattedDisplayDate = date ? format(parseISO(date), 'dd/MM/yyyy') : 'Invalid Date'

  return (
    <YStack flex={1} style={{ backgroundColor: (theme.background?.val as string) || 'white' }}>
      ...
      <ScrollView>
        ...
        {dayExpenses.length === 0 ? (
           <Text style={{ color: (theme.gray10?.val as string) || 'gray', textAlign: 'center', marginTop: 20 }}>No expenses found for this date.</Text>
        ) : (
