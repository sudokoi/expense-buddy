...
describe("Version Code Decoding", () => {
  const stableVersionArb = fc
    .tuple(
      fc.integer({ min: 0, max: 99 }),
      fc.integer({ min: 0, max: 99 }),
      fc.integer({ min: 0, max: 99 })
    ...
    .map(([major, minor, patch]) => ({
      ...
      versionCode: major * 10000000 + minor * 100000 + patch * 1000 + 999,
    }))

  it("stable version codes SHALL decode back to the original semantic version", () => {
    fc.assert(
      fc.property(stableVersionArb, ({ version, versionCode }) => {
        expect(decodeVersionCode(versionCode)).toBe(version)
        return true
      }),
      { numRuns: 100 }
    )
  ...
  it("invalid version codes SHALL return null", () => {
    fc.assert(
      fc.property(fc.integer({ max: 0 }), (versionCode) => {
        expect(decodeVersionCode(versionCode)).toBeNull()
        return true
      }),
      { numRuns: 50 }
    )
