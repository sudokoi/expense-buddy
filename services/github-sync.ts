...
export async function validatePAT(
  ...
  try {
    ...
    if (!owner || !repoName) {
      return { valid: false, error: 'Invalid repository format. Use: username/repo' };
    }

    console.log('Testing connection to:', `https://api.github.com/repos/${owner}/${repoName}`);
    ...
    if (response.status === 404) {
      return { valid: false, error: 'Repository not found or token lacks access. Check: 1) Repo name is correct 2) Token has Contents permission 3) Token can access this specific repo' };
    }
    ...
    if (response.status === 403) {
      ...
      return { valid: false, error: 'Access forbidden. Token may lack required permissions (Contents: Read and Write)' };
    }

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.log('API Error:', errorData);
      return { valid: false, error: `GitHub API error (${response.status}): ${errorData.message || response.statusText}` };
    }
