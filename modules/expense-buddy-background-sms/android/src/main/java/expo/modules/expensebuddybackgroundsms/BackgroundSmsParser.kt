...
private val otpKeywords = Regex("\\botp\\b|one[ -]?time password|verification code|security code|auth(?:entication)? code|passcode|do not share|never share|valid for \\d+ (?:minute|min|minutes|mins)", RegexOption.IGNORE_CASE)
private val nonExpenseInfoKeywords = Regex("available balance|avl(?:\\.|\\s)?bal|a/c balance|account balance|balance is|ledger balance|min(?:imum)? due|total due|payment due|due date|bill(?:ing)? statement|statement generated|statement ready|e-?statement|autopay|auto-debit mandate|standing instruction|card ending|card blocked|card hotlisted|card limit|credit limit|cash limit|cvv|pin|mpin|tpin|token(?:isation|ization)?|token generated|registered for e-?com|e-?commerce|online usage enabled|international usage enabled|contactless usage enabled", RegexOption.IGNORE_CASE)
private val nonExpenseTransactionOutcomeKeywords = Regex("declined due to|was declined|failed due to|unsuccessful|reversed|reversal|refund initiated|chargeback|no amount debited", RegexOption.IGNORE_CASE)
private val approvalPromptKeywords = Regex("if not you|if this wasn'?t you|approve|approval|authenticate|authorize|authorise|confirm this transaction|complete this transaction|to complete your transaction|to proceed", RegexOption.IGNORE_CASE)
private val merchantPattern = Regex("\\b(?:at|to|merchant)\\s+([A-Za-z0-9&._\\-/ ]{2,40})", RegexOption.IGNORE_CASE)

private val categoryInferenceRules = listOf(
  "Food" to Regex("swiggy|zomato|restaurant|restro|cafe|coffee|pizza|burger|biryani|dining|eatery|bakery|food", RegexOption.IGNORE_CASE),
  "Transport" to Regex("uber|ola|rapido|metro|rail|train|irctc|bus|cab|taxi|petrol|diesel|fuel|parking|toll|travel", RegexOption.IGNORE_CASE),
  "Groceries" to Regex("grocery|groceries|supermarket|hypermarket|bigbasket|blinkit|zepto|instamart|fresh|dmart|reliance fresh", RegexOption.IGNORE_CASE),
  "Rent" to Regex("\\brent\\b|landlord|lease|tenancy|apartment rent|house rent", RegexOption.IGNORE_CASE),
  "Utilities" to Regex("electricity|water bill|utility bill|gas bill|broadband|wifi|internet bill|mobile bill|recharge|airtel|jio|vi\\b|bsnl", RegexOption.IGNORE_CASE),
  "Entertainment" to Regex("netflix|spotify|prime video|hotstar|bookmyshow|movie|cinema|theatre|gaming|playstation|xbox", RegexOption.IGNORE_CASE),
...
object BackgroundSmsParser {
  ...
  private fun inferCategory(body: String, merchantName: String?): String {
    val normalizedContent = listOfNotNull(merchantName, body)
    ...
    return categoryInferenceRules.firstOrNull { (_, pattern) ->
