...
- Summary, seven-day trend, and recent rows are lazy, refresh-local projections: reading one does not calculate the others. The returned data keeps a fixed date and selected live rows, not a persistent cache.
- Provider broadcasts join a process-owned render queue with a fixed 25ms collection window. Duplicate installed widget targets merge, and requests arriving during a render form a trailing batch with a new live snapshot. Each batch shares that snapshot across provider kinds; Recent's collection factory still reads live data on its system-controlled callback. No snapshot survives between batches and `max(updatedAt)` is never a cache key.
- Broadcasts retain `goAsync` ownership until their shared batch completes, with an eight-second waiter timeout. Rendering is serial, with a seven-second coroutine timeout per batch and cancellation checks between widget instances; a synchronous native call may run until its next cancellation boundary. Cancelling one waiter does not cancel other broadcasts' work. Targets are checked against installed IDs both before enqueue and before render, so removed widgets are skipped.
- One narrow `WidgetAssist` reader (`fun load(): Assist?`).
- Three thin provider Adapters at the `RemoteViews` seam: `Summary`, `Trend7d` (7 category-stacked bars drawn to a bitmap with `Canvas` — `RemoteViews` cannot host chart views; zero-filled with faint track slots and a monotone cubic total trend line that passes through every daily value without overshoot), `Recent` (`RemoteViewsService` list, capped at 8). Category order and colors come from live `app_settings`; assist colors are degraded fallback only. Summary identifies a selected category with its dot and label, while Recent uses the same native style owner for row dots.
