...
| Native modules     | Android SMS access, background SMS alerts, Play Core, structured logging, and home-screen widgets | `modules/expense-buddy-sms-parser/`, `modules/expense-buddy-sms-module/`, `modules/expense-buddy-play-core/`, `modules/expense-buddy-logger/`, `modules/expense-buddy-widget/` |
...
- [ADR-003: Regex-First SMS Import](./decisions/adr-003-regex-first-sms-import.md)
...
- [ADR-001: On-Device ML SMS Categorization](./decisions/adr-001-on-device-ml-sms-categorization.md)
...
- SMS import uses region rule packs (India, US, UK, Canada, Australia, and Japan) selected by an explicit `smsRegion` setting that seeds from the device locale, cascades on language change, and falls back to India for unknown values (see [ADR-010](./decisions/adr-010-region-packs-sms-import.md)).
